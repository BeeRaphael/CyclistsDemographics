...
### The Question:
   ...
   large - on an individual scale as well as on a global scale.
   
   While this exploratory data analyses gives us a first glimpse it can not answer all those questions. A lot more data over a larger time frame need to be pulled together to get a good overview.  
   
### Project proposal
   One idea to increase the number of cycling commuters is to have a Cycling Commuter Challenge between several companies e. g. within the Silicon Valley. I propose as a project for the Data Incubator fellowship to design an app that would allow companies, teams, and individual cyclist to log their cycling to work miles and compare them with other teams.
   A more advanced version would be to collect tracking data from smartphones and fitness devices like Fitbits.
   (Though I am not sure if something like this already exists.)
